...
@Component({
  ...
  template: `
    <div
      class="absolute inset-0.5 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer select-none"
      [class.ring-2]="isSelected() || isCurrentTurn()"
      [class.ring-amber-400]="isCurrentTurn()"
      [class.ring-white]="isSelected() && !isCurrentTurn()"
      [class.scale-110]="isSelected()"
      [class.opacity-50]="!isCurrentTurn() && hasInitiative()"
      [title]="character().name"
    >
      <img
        [src]="classImage()"
        [alt]="character().class"
        class="w-full h-full object-contain rounded-full bg-stone-900/80 p-0.5"
